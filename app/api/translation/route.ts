...
export async function POST(request: NextRequest) {
  try {
    ...
    const userPrompt = `Translate the following text from ${sourceLanguage || 'Auto Detect'} to ${targetLanguage}. ${preserveStyle ? 'Preserve the literary style, rhythm, and tone as closely as possible.' : 'Prioritize clarity and natural flow in the target language.'} After the translation, provide a brief section called TRANSLATOR'S NOTE that explains: 1) what literary elements were preserved, 2) what was inevitably lost or changed in translation, 3) any interesting linguistic or cultural nuances the reader should know about. Format your response exactly as: TRANSLATION: [the translated text] TRANSLATOR'S NOTE: [your notes] Text to translate: ${text}`;
    ...
    for (const key of splitKeys) {
      const idx = responseText.indexOf(key);
      if (idx !== -1) {
        splitIndex = idx;
        foundKey = key;
        break;
    ...
    if (splitIndex !== -1) {
      const beforeNote = responseText.slice(0, splitIndex).trim();
      const afterNote = responseText.slice(splitIndex + foundKey.length).trim();

      // Clean up "TRANSLATION:" prefix if present
      translation = beforeNote.replace(/^TRANSLATION:\s*/i, '').trim();
      translatorsNote = afterNote;
    } else {
      translation = responseText.replace(/^TRANSLATION:\s*/i, '').trim();
      translatorsNote = "No translator's note was generated.";
    }

    return NextResponse.json({ translation, translatorsNote });
  } catch (error) {
